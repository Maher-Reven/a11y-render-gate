...
- **Source mapping** — findings carry the offending element's classes, but not a
  `file:line`.
- **Windows** — CI covers Linux; developed on macOS.
- **Sites behind bot protection** may still refuse to load even with a custom
  user-agent, if they use a JavaScript challenge.
